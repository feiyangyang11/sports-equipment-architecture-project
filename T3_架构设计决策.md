...
| AD-07 | 预留关键操作留痕扩展点 | 审计日志预留模式 | 本版本暂不实现操作日志，为后续可审计性扩展预留位置 |
...
| 数据层 | 保存用户、器材、预约、借还等核心业务数据，并为后续日志数据预留扩展空间 |
...
对于“审核通过后的借出办理”“归还并回收库存”等复合操作，后端需要在数据库事务中完成以下步骤：

1. 校验当前记录状态。
2. 锁定或读取器材库存。
3. 更新预约或借用状态。
4. 修改库存数量。
5. 如后续启用操作日志模块，再补充日志写入步骤。
...
- 管理员：允许维护器材、审核预约、办理借还、查看统计；操作日志能力待后续添加

接口层在进入业务逻辑前执行角色校验，防止普通学生越权调用管理员接口。

### AD-07 预留操作日志扩展能力

本版本暂不实现独立的操作日志模块，但为了避免后续扩展时推翻现有结构，仍在架构设计中预留相关扩展点。后续如补充操作日志，典型记录场景包括：
...
日志字段建议至少包含：操作人、时间、操作类型、目标对象、摘要、结果。本版本先不落地这部分编码与接口，仅保留设计位。

### AD-08 采用关系型数据库存储核心业务数据

系统中的器材、库存、预约、借用和日志天然具有关联关系和一致性要求，因此采用 MySQL 作为主存储更合适。
...
| `operation_log` | 管理操作审计日志预留表，待后续版本启用 |
...
归还流程中，如果器材正常归还，则回收库存并关闭借用记录；如果借用已经逾期，则在归还办理时同步更新借用状态或备注，保证后续查询与统计一致。
...
| 安全性 | AD-02、AD-06、AD-07 | 通过统一接口入口、RBAC 实现当前版本权限控制，并为后续日志审计预留扩展能力 |
...
| 可审计性 | AD-07 | 当前版本先保留统一日志扩展位，后续再将关键管理行为纳入统一日志 |
...
本架构设计选择了“前后端分离 + RESTful 接口 + 分层单体 + 事务一致性 + RBAC”的总体方案，并为操作日志保留后续扩展位置。该方案在课程项目范围内兼顾了实现可行性、业务完整性和架构分析价值，为 T4 的 ATAM 评估提供了明确的设计对象。
